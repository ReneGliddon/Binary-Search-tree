...
class binaryTreeType
{
    public:
        ...
        int recNumOfRightChildren(binaryTreeNode<elemType> *p);
        int rightChildNum;
        int callNodeCount();
    ...
    private:
        ...
        int nodeCount(binaryTreeNode<elemType> *p) const;
            //Function to return the number of nodes in the binary tree to which p points
        void recNodeCount(binaryTreeNode<elemType>*p, int &counter) const;
            //the recursive function associated with nodeCount to count the number of nodes in a tree
        int leavesCount(binaryTreeNode<elemType> *p) const;
            //Function to return the number of leaves in the binary tree to which p points
        int numofRightChildren(binaryTreeNode<elemType> *p) const;
            //function to return the number of right children
};

template<class elemType>
int binaryTreeType<elemType>::callNodeCount()
{
    nodeCount(root);
}
...
{
    int nodeCounter;
    //initialise the count to 1 if root is not null
    if(p!=NULL)
    {
        nodeCounter=1;
        recNodeCount(p,nodeCounter);
    }
...
template <class elemType>
void binaryTreeType<elemType>::recNodeCount(binaryTreeNode<elemType>*p, int &counter) const
{
    ...
    //
        ...
        {
            counter++;
            recNodeCount(p->llink, counter);
        }

        if(p->rlink!=NULL)
        {
            counter++;
            recNodeCount(p->rlink, counter);
        }
...
template <class elemType>
int binaryTreeType<elemType>::numberOfRightChildren()
{
    this->rightChildNum = 0;
    int numberOfRightChildren = recNumOfRightChildren(root);

    return numberOfRightChildren;
//return rightChildNum;
}

//int recNumOfRightChildren(binaryTreeType<elemType> *p, int &rightChildNum);
template <class elemType>
int binaryTreeType<elemType>::recNumOfRightChildren(binaryTreeNode<elemType> *p)
{
    if (this->isEmpty())
        return 0;
    if(p->rlink!=NULL)
    {
        this->rightChildNum++;
        if(p->rlink!=NULL)
            recNumOfRightChildren(p->rlink);
        if(p->llink!=NULL)
            recNumOfRightChildren(p->llink);
